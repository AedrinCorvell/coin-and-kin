...
const TR_ITEM = {
  ...
  holy_armor:"Kutsal Zırh", bone_relic:"Kemik Emaneti", shadow_cloak:"Gölge Pelerini", beast_horn:"Vahşi Boynuz",
...
const TR_CLASS = {
  ...
  Paladin:"Şövalye", Necromancer:"Nekromansör", Assassin:"Suikastçı", Beastmaster:"Canavar Efendisi",
};
const TR_ROLE = {
...
function traitName(n){ if(LANG==="tr" && TR_TRAIT[n]) return TR_TRAIT[n]; return n; }
const TR_BUILDING = {
...
const TR_BUILDING_DESC = {
  dormitory:"Maceracılar burada kalmak için sana ödeme yapar. Daha iyi yataklar = daha çok kira geliri, mutluluk ve kapasite.",
  ...
  treasury:"Lonca kademesini yükseltir, yeni bölgeleri ve üyeleri açar.",
};
...
function traitDesc(n){ if(LANG==="tr" && TR_TRAIT_DESC[n]) return TR_TRAIT_DESC[n]; return n; }
const TR_REL = {
      ...
      setState(s=>{
        ...
        if(ns.elapsed % 5 === 0){
          ...
          const adv3=(ns.adventurers||s.adventurers).map(a=>{
            ...
            else if(t==="patrol"){ patrolGold+=1; patrolRenownAcc+=0.1; taskChanged=true; }
            return na;
          });
...
function TopBar({state}){
  ...
  return (<header style={{position:"sticky",top:0,zIndex:40,background:`linear-gradient(${T.panel2},${T.panel})`,
    ...
    <div style={{maxWidth:760,margin:"0 auto",display:"flex",alignItems:"center",gap:12}}>
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:15,fontWeight:700,letterSpacing:.5}}>{state.guildName||t("top.defaultName")}</div>
        <div style={{fontSize:11,color:rank.color}}>{t("guildrank."+tier)}</div>
      </div>
      <div style={{position:"relative",width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center"}} title={t("top.nextPayday")+" "+fmtTime(secsLeft)}>
        <DayClock progress={progress}/>
        <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          <span style={{fontSize:8,color:T.inkFaint,lineHeight:1}}>{t("top.day")}</span>
...
function HallView({state,setTab,setState,notify,newGame,lang,switchLang}){
  ...
  return (<div style={{animation:"fadeUp .3s"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
      <StatCard icon="⚔️" label={t("hall.adventurers")} value={`${state.adventurers.length}/${cap}`} sub={`${idle} ${t("hall.idle")} · ${questing} ${t("hall.away")}`} onClick={()=>setTab("advs")}/>
      <StatCard icon="🗺️" label={t("hall.expeditions")} value={questing} sub={t("hall.tapDispatch")} onClick={()=>setTab("quests")}/>
      <StatCard icon="🗝️" label={t("hall.storage")} value={invCount} sub={t("hall.itemsHeld")} onClick={()=>setTab("storage")}/>
      <StatCard icon="⚒️" label={t("hall.crafting")} value={state.crafting.length} sub={t("hall.inProgress")} onClick={()=>setTab("craft")}/>
    </div>
  ...
  return (<Panel style={{marginBottom:10}}>
    <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:11,color:T.inkDim,marginBottom:5}}>Lv {a.level} {a.gender||"♂"} {clsName(a.cls)} {c.rare?<span style={{color:T.gold,fontWeight:700}}>★ {t("adv.rare")}</span>:""} · {roleName(c.role)} · {statusLabel(a)}</div>
...
function AdvDetailModal({a,rels,roster,setState,onClose,onDismiss}){
  ...
  return (<Modal title={editName?t("adv.rename"):a.name} onClose={onClose}>
    ...
    <SectionTitle>{t("adv.traits")} {a.pos2 && <span style={{color:T.gold,fontSize:10}}>★ {t("adv.twoVirtues")}</span>}</SectionTitle>
...
function QuestsView({state,setState,notify}){
  ...
  const dispatch=(region,partyIds,repeat)=>{
    ...
    setState(s=>({...s,
      ...
      lastParty:partyIds,
      log:[{t:s.elapsed,msg:LANG==="tr"?`${partyIds.length} kişi ${regionName(region)} bölgesine yollandı${rep>1?` ×${rep}`:""} (~${fmtTime(dur)}${rep>1?" her biri":""}).`:`Sent ${partyIds.length} to ${regionName(region)}${rep>1?` ×${rep}`:""} (~${fmtTime(dur)}${rep>1?" each":""}).`},...s.log].slice(0,40)}));
    setPicking(null); notify(LANG==="tr"?`Sefer ${regionName(region)} bölgesine yollandı${rep>1?` (×${rep})`:""}.`:`Expedition dispatched to ${regionName(region)}${rep>1?` (×${rep})`:""}.`); Audio.play("quest");
  };
      ...
      {state.quests.map(q=>{ const r=REGIONS.find(x=>x.id===q.regionId); const total=q.endAt-q.startAt; const pct=(state.elapsed-q.startAt)/total*100;
        return (<Panel key={q.id} style={{marginBottom:10}}>
          <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{regionName(r)}</div>
...
function PartyPicker({region,idle,lastParty,rels,tavernSucc,onClose,onGo}){
  ...
  return (<Modal title={`${t("quest.dispatch")} — ${regionName(region)}`} onClose={onClose}>
    <div style={{fontSize:12,color:T.inkDim,marginBottom:10}}>{t("pp.intro")}</div>
    ...
    {idle.map(a=>{ const c=CLASSES[a.cls]; const on=sel.includes(a.id);
      return (<button key={a.id} onClick={()=>toggle(a.id)} style={{width:"100%",textAlign:"left",marginBottom:8,
        ...
        <div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>{a.name}</div>
          <div style={{fontSize:10,color:T.inkDim}}>Lv {a.level} {clsName(a.cls)} · {roleName(c.role)} · <span style={{color:T.green}}>{clsPerk(a.cls)}</span></div></div>
    ...
    <GoldBtn onClick={()=>onGo(region,sel,repeat)} disabled={sel.length===0} style={{width:"100%",marginTop:6}}>
      {t("pp.send")} {sel.length>0?`${sel.length}`:""}{repeat>1?` · ${t("pp.repeatX")} ×${repeat}`:""} ›</GoldBtn>
  </Modal>);
}
...
function UpgradesView({state,setState,notify}){
  const upgrade=(key)=>{ const b=BUILDINGS[key]; const cur=state.buildings[key]; const next=cur+1;
    ...
    setState(s=>({...s,gold:s.gold-cost,buildings:{...s.buildings,[key]:next},
      log:[{t:s.elapsed,msg:LANG==="tr"?`${buildingName(key)} Lv ${next} seviyesine yükseltildi.`:`Upgraded ${buildingName(key)} to Lv ${next}.`},...s.log].slice(0,40)}));
    notify(LANG==="tr"?`${buildingName(key)} yükseltildi!`:`${buildingName(key)} upgraded!`);
  };
  return (<div style={{animation:"fadeUp .3s"}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          ...
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between"}}>
              <div style={{fontWeight:700,fontSize:14}}>{buildingName(key)}</div>
...
function describeBuilding(key,cur,next){
  const fmt=(lv)=>{
    if(key==="dormitory")return LANG==="tr"?`kapasite ${lv.capacity}, kira ${lv.rentPerHead}A/kişi`:`cap ${lv.capacity}, rent ${lv.rentPerHead}G/head`;
    ...
    if(key==="treasury")return LANG==="tr"?`lonca kademesi ${lv.tier}`:`guild tier ${lv.tier}`;
